...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, input_size=224):
        ...
        # For ImageNet (224x224), use kernel_size=7, stride=2 for first conv
        # For CIFAR (32x32), use kernel_size=3, stride=1 for first conv
        if input_size >= 224:  # ImageNet
            self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
            self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
            self.avgpool_size = 7  # 224 -> 112 -> 56 -> 28 -> 14 -> 7
        else:  # CIFAR
            self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
            self.maxpool = nn.Identity()
            self.avgpool_size = 4  # 32 -> 32 -> 16 -> 8 -> 4
...
def ResNet50(num_classes=1000, input_size=224):
    ...
    Args:
        ...
        input_size (int): Input image size. 224 for ImageNet, 32 for CIFAR.
